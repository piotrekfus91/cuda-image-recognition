...
class UnionFindSegmentator : public ::testing::Test {
protected:
...
void UnionFindSegmentator::SetUp() {
	_logger = new NullLogger;
	_service = new CpuImageProcessingService(*_logger);
	_service->setSegmentator(new CpuUnionFindSegmentator);
	_service->setSegmentatorMinSize(0);
}

void UnionFindSegmentator::TearDown() {
	delete _logger;
	delete _service;
}

TEST_F(UnionFindSegmentator, Sample) {
...
TEST_F(UnionFindSegmentator, Sample2) {
...
TEST_F(UnionFindSegmentator, Sample9x11) {
...
void compare_using_region_growing_and_union_find(const char* fileName, CpuImageProcessingService* _service, Logger* _logger) {
...
TEST_F(UnionFindSegmentator, Metro) {
	compare_using_region_growing_and_union_find("metro_red_yellow.bmp", _service, _logger);
}
